...
import ly.plugins.vap_player.opengl.FileUtils
 ...
 */
class VideoRenderer(textureId: Int, surface: Surface, context: Context) : ExternalGLThread.Renderer {
    ...
    private fun initPlayer(surface: Surface) {
        val videoFile = File(FileUtils.getExternalAssetsDir(mContext), "sample.mp4")
        var mediaItem = MediaItem.fromUri(Uri.fromFile(videoFile))
        var exoPlayer = ExoPlayer.Builder(mContext).build();
    ...
    override fun onCreate() {
        ...
        // Image CenterInside
        // Image CenterInside
        mMvpMatrix = GLESUtils.changeMvpMatrixInside(viewportWidth.toFloat(), viewportHeight.toFloat(), mBitmapWidth.toFloat(), mBitmapHeight.toFloat())
    }
