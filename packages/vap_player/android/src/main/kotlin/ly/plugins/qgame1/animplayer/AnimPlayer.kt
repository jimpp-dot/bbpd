...
import ly.plugins.qgame1.animplayer.plugin.AnimPluginManager
import ly.plugins.qgame1.animplayer.util.ALog

class AnimPlayer(val animView: IAnimView, textureId: Int) {
    ...
    private fun innerStartPlay(fileContainer: IFileContainer) {
        synchronized(AnimPlayer::class.java) {
            if (isSurfaceAvailable) {
                animView.getSurfaceTexture()?.setDefaultBufferSize(configManager.config!!.width, configManager.config!!.height)
