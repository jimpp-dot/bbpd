...
//fragment float4 vap_rgbFragmentShader(VAPRasterizerData input [[stage_in]],
...
//    
//    half4 colorSample = colorTexture.sample(textureSampler, input.textureColorCoordinate); // 得到纹理对应位置的颜色
//    
...
fragment float4 vap_rgbFragmentShader(VAPRasterizerData input [[ stage_in ]],
    ...
    constexpr sampler textureSampler (mag_filter::linear, min_filter::linear);
//    return colorTexture.sample(textureSampler, input.textureColorCoordinate); // 得到纹理对应位置的颜色
    float3 rgbColorSample = colorTexture.sample(textureSampler, input.textureColorCoordinate).rgb;
    float alphaColorSample = colorTexture.sample(textureSampler, input.textureAlphaCoordinate).r;
