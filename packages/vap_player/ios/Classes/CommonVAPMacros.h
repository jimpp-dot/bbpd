...
#import "CommonQGVAPMaskInfo.h"

extern NSInteger const kCQGHWDMP4DefaultFPS;      //默认fps 25
extern NSInteger const kCQGHWDMP4MinFPS;          //最小fps 1
extern NSInteger const kCQGHWDMP4MaxFPS;          //最大fps 60

extern NSInteger const kCVapMaxCompatibleVersion; //最大兼容版本

@class CommonQGVAPSourceDisplayItem;

//typedef UIView VAPView; //特效播放容器

/* mp4素材中每一帧alpha通道数据的位置*/
typedef NS_ENUM(NSInteger, CommonQGHWDTextureBlendMode){
    
    CommonQGHWDTextureBlendMode_AlphaLeft                 = 0,          // 左侧alpha
    CommonQGHWDTextureBlendMode_AlphaRight                = 1,          // 右侧alpha
    CommonQGHWDTextureBlendMode_AlphaTop                  = 2,          // 上侧alpha
    CommonQGHWDTextureBlendMode_AlphaBottom               = 3,          // 下测alpha
};

typedef void(^VAPImageCompletionBlock)(UIImage * image, NSError * error,NSString *imageURL);

typedef void(^VAPGestureEventBlock)(UIGestureRecognizer *gestureRecognizer, BOOL insideSource, CommonQGVAPSourceDisplayItem *source);
