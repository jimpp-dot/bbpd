...
#import <simd/simd.h>
#import "UIDevice+VAPUtilTexture.h"
#import "CommonQGVAPMetalUtil.h"
#import "CommonQGVAPMetalShaderFunctionLoader.h"

#if TARGET_OS_SIMULATOR//模拟器
#else

@interface CommonQGVAPMetalRenderer () {
...
@property (nonatomic, strong) CommonQGVAPMetalShaderFunctionLoader *shaderFuncLoader;

@end

@implementation CommonQGVAPMetalRenderer
{
    CVPixelBufferRef _targetPixelBuffer;
}

//kQGHWDMetalRendererDevice
- (instancetype)initWithSize:(CGSize)size {
    
    if (self = [super init]) {
        if (!kCQGHWDMetalRendererDevice) {
            kCQGHWDMetalRendererDevice = MTLCreateSystemDefaultDevice();
        }
        _size = size;
...
- (void)setupRenderContext {
    ...
    //constants
    _currentColorConversionMatrix = kCColorConversionMatrix601FullRangeDefault;
    ...
    _yuvMatrixBuffer = [kCQGHWDMetalRendererDevice newBufferWithBytes:yuvMatrixs length:yuvMatrixsDataSize options:kDefaultMTLResourceOption];
    
    //function loader
    self.shaderFuncLoader = [[CommonQGVAPMetalShaderFunctionLoader alloc] initWithDevice:kCQGHWDMetalRendererDevice];
    
    //command queue
    self.commandQueue = [kCQGHWDMetalRendererDevice newCommandQueue];
    
    //texture cache
    CVReturn textureCacheError = CVMetalTextureCacheCreate(kCFAllocatorDefault, nil, kCQGHWDMetalRendererDevice, nil, &_videoTextureCache);
...
- (void)renderPixelBuffer:(CVPixelBufferRef)pixelBuffer metalLayer:(CAMetalLayer *)layer mergeInfos:(NSArray<CommonQGVAPMergedInfo *> *)infos {
    ...
    CVReturn ret = CVMetalTextureCacheCreateTextureFromImage(kCFAllocatorDefault, _videoTextureCache, _targetPixelBuffer, nil, MTLPixelFormatBGRA8Unorm, self.size.width, self.size.height, 0, &renderTargetMetalTextureRef);
    if (ret != kCVReturnSuccess) {
        VAP_Error(kQGVAPModuleCommon, @"CVMetalTextureCacheCreateTextureFromImage%@", @(ret));
        return ;
    }
    id<MTLTexture> mtlTexture = CVMetalTextureGetTexture(renderTargetMetalTextureRef);
    CVBufferRelease(renderTargetMetalTextureRef);
    renderTargetMetalTextureRef = NULL;

    MTLRenderPassDescriptor *renderPassDescriptor = [MTLRenderPassDescriptor new];
    renderPassDescriptor.colorAttachments[0].texture = mtlTexture; //which returns the texture in which you need to draw in order for something to appear on the screen.
    ...
    [self drawMergedAttachments:infos yTexture:yTexture uvTexture:uvTexture renderEncoder:renderEncoder];
    [renderEncoder endEncoding];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"pixelBuffer" object:
     @{@"pixelBuffer": (__bridge id _Nullable)(self->_targetPixelBuffer),
...
- (void)dispose {
    ...
    if (_targetPixelBuffer) {
        CVPixelBufferRelease(_targetPixelBuffer);
        _targetPixelBuffer = NULL;
    }
}
...
- (void)drawMergedAttachments:(NSArray<CommonQGVAPMergedInfo *> *)infos
    ...
    [infos enumerateObjectsUsingBlock:^(CommonQGVAPMergedInfo * _Nonnull mergeInfo, NSUInteger idx, BOOL * _Nonnull stop) {

        [encoder setRenderPipelineState:self.attachmentPipelineState];
        id<MTLTexture> sourceTexture = mergeInfo.source.texture;//图片纹理
        id<MTLBuffer> vertexBuffer = [mergeInfo vertexBufferWithContainerSize:self.commonInfo.size maskContianerSize:self.commonInfo.videoSize device:kCQGHWDMetalRendererDevice];
...
- (id<MTLBuffer>)maskBlurBuffer {
    if (!_maskBlurBuffer) {
        struct MaskParameters parameters[] = {{kCBlurWeightMatrixDefault, 3, 0.01}};
        NSUInteger parametersSize = sizeof(struct MaskParameters);
        _maskBlurBuffer = [kCQGHWDMetalRendererDevice newBufferWithBytes:parameters length:parametersSize options:kDefaultMTLResourceOption];
    }
...
//    pipelineStateDescriptor.colorAttachments[0].pixelFormat = _metalLayer.pixelFormat;
    ...
    NSError *psError = nil;
    id<MTLRenderPipelineState> pipelineState = [kCQGHWDMetalRendererDevice newRenderPipelineStateWithDescriptor:pipelineStateDescriptor error:&psError];
...
- (id<MTLRenderPipelineState>)defaultMainPipelineState {
    if (!_defaultMainPipelineState) {
        _defaultMainPipelineState = [self createPipelineState:kCVAPVertexFunctionName fragmentFunction:kCVAPYUVFragmentFunctionName];
    }
...
- (id<MTLRenderPipelineState>)mainPipelineStateForMask {
    if (!_mainPipelineStateForMask) {
        _mainPipelineStateForMask = [self createPipelineState:kCVAPVertexFunctionName fragmentFunction:kCVAPMaskFragmentFunctionName];
    }
...
- (id<MTLRenderPipelineState>)attachmentPipelineState {
    if (!_attachmentPipelineState) {
        _attachmentPipelineState = [self createPipelineState:kCVAPAttachmentVertexFunctionName fragmentFunction:kCVAPAttachmentFragmentFunctionName];
    }
...
- (id<MTLRenderPipelineState>)mainPipelineStateForMaskBlur {
    if (!_mainPipelineStateForMaskBlur) {
        _mainPipelineStateForMaskBlur = [self createPipelineState:kCVAPVertexFunctionName fragmentFunction:kCVAPMaskBlurFragmentFunctionName];
    }
...
- (void)updateMetalPropertiesIfNeed:(CVPixelBufferRef)pixelBuffer {
    ...
    matrix_float3x3 matrix = kCColorConversionMatrix601FullRangeDefault;
    if (CFStringCompare(yCbCrMatrixType, kCVImageBufferYCbCrMatrix_ITU_R_709_2, 0) == kCFCompareEqualTo) {
        matrix = kCColorConversionMatrix709FullRangeDefault;
    }
...
- (void)updateMainVertexBuffer {
    ...
    const void *vertices = kCVAPMTLVerticesIdentity;

    commonGenMTLTextureCoordinates(self.commonInfo.rgbAreaRect, self.commonInfo.videoSize, rgbCoordinates, NO, 0);
    commonGenMTLTextureCoordinates(self.commonInfo.alphaAreaRect, self.commonInfo.videoSize, alphaCoordinates, NO, 0);
    if (self.maskInfo) {
        commonGenMTLTextureCoordinates(self.maskInfo.sampleRect, self.maskInfo.dataSize, maskCoordinates, NO, 0);
    }
    ...
    id<MTLBuffer> vertexBuffer = [kCQGHWDMetalRendererDevice newBufferWithBytes:vertexData length:allocationSize options:kDefaultMTLResourceOption];
