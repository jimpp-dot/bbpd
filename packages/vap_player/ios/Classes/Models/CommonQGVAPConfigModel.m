...
//资源适配类型
CommonQGAGAttachmentFitType       const kCQGAGAttachmentFitTypeFitXY          = @"fitXY";         //按指定尺寸缩放
CommonQGAGAttachmentFitType       const kCQGAGAttachmentFitTypeCenterFull     = @"centerFull";    //默认按资源尺寸展示，如果资源尺寸小于遮罩，则等比缩放至可填满
//资源类型
CommonQGAGAttachmentSourceType    const kCQGAGAttachmentSourceTypeTextStr     = @"textStr";       //文字
CommonQGAGAttachmentSourceType    const kCQGAGAttachmentSourceTypeImgUrl      = @"imgUrl";        //图片

CommonQGAGAttachmentSourceType    const kCQGAGAttachmentSourceTypeText        = @"txt";       //文字
CommonQGAGAttachmentSourceType    const kCQGAGAttachmentSourceTypeImg         = @"img";        //图片
CommonQGAGAttachmentSourceLoadType const kCQGAGAttachmentSourceLoadTypeLocal   = @"local";
CommonQGAGAttachmentSourceLoadType const kCQGAGAttachmentSourceLoadTypeNet     = @"net";

//字体
CommonQGAGAttachmentSourceStyle   const kCQGAGAttachmentSourceStyleBoldText   = @"b";             //粗体


@implementation CommonQGVAPConfigModel
...
@implementation CommonQGVAPCommonInfo
...
@implementation CommonQGVAPSourceInfo
...
@implementation CommonQGVAPSourceDisplayItem

@end

@implementation CommonQGVAPMergedInfo
...
- (id<MTLBuffer>)vertexBufferWithContainerSize:(CGSize)size maskContianerSize:(CGSize)mSize device:(id<MTLDevice>)device {
    ...
    commonGenMTLVertices(self.renderRect, size, vertices, NO);
    commonGenMTLTextureCoordinates(self.maskRect, mSize, maskCoordinates,YES, self.maskRotation);
    
    if ([self.source.fitType isEqualToString:kCQGAGAttachmentFitTypeCenterFull]) {
        CGRect sourceRect = commonVapRectForCenterFull(self.source.size, self.renderRect.size);
        CGSize sourceSize = commonVapSourceSizeForCenterFull(self.source.size, self.renderRect.size);
        commonGenMTLTextureCoordinates(sourceRect, sourceSize, sourceCoordinates,NO, 0);
    } else {
        commonReplaceArrayElements(sourceCoordinates, kCVAPMTLTextureCoordinatesIdentity, 8);
    }
