...
@interface CommonQGMP4FrameHWDecoder() {
    ...
    CommonQGMP4ParserProxy *_mp4Parser;
...
NSString *const CommonQGMP4HWDErrorDomain = @"CommonQGMP4HWDErrorDomain";

@implementation CommonQGMP4FrameHWDecoder
...
+ (NSString *)errorDescriptionForCode:(CommonQGMP4HWDErrorCode)errorCode {
    ...
    NSString *desc = @"";
    switch (errorCode) {
        case CommonQGMP4HWDErrorCode_FileNotExist:
            desc = [errorDescs safeObjectAtIndex:0];
            break;
        case CommonQGMP4HWDErrorCode_InvalidMP4File:
            desc = [errorDescs safeObjectAtIndex:1];
            break;
        case CommonQGMP4HWDErrorCode_CanNotGetStreamInfo:
            desc = [errorDescs safeObjectAtIndex:2];
            break;
        case CommonQGMP4HWDErrorCode_CanNotGetStream:
            desc = [errorDescs safeObjectAtIndex:3];
            break;
        case CommonQGMP4HWDErrorCode_ErrorCreateVTBDesc:
            desc = [errorDescs safeObjectAtIndex:4];
            break;
        case CommonQGMP4HWDErrorCode_ErrorCreateVTBSession:
...
- (instancetype)initWith:(CommonQGMP4HWDFileInfo *)fileInfo error:(NSError *__autoreleasing *)error{
...
- (void)handleDecodePixelBuffer:(CVPixelBufferRef)pixelBuffer
    ...
    CommonQGMP4AnimatedImageFrame *newFrame = [[CommonQGMP4AnimatedImageFrame alloc] init];
    ...
    // 9. sort
    [_buffers sortUsingComparator:^NSComparisonResult(CommonQGMP4AnimatedImageFrame * _Nonnull obj1, CommonQGMP4AnimatedImageFrame * _Nonnull obj2) {
...
- (BOOL)onInputStart {
    ...
    if (![fileMgr fileExistsAtPath:self.fileInfo.filePath]) {
        _constructErr = [NSError errorWithDomain:CommonQGMP4HWDErrorDomain code:CommonQGMP4HWDErrorCode_FileNotExist userInfo:[self errorUserInfo]];
        return NO;
    }
...
- (BOOL)initPPSnSPS {
    ...
    if (self.spsData != nil && self.ppsData != nil && _mp4Parser.videoCodecID != CommonQGMP4VideoStreamCodecIDUnknown) {
        if (_mp4Parser.videoCodecID == CommonQGMP4VideoStreamCodecIDH264) {
            ...
            if (_status != noErr) {
                ...
                _constructErr = [NSError errorWithDomain:CommonQGMP4HWDErrorDomain code:CommonQGMP4HWDErrorCode_ErrorCreateVTBDesc userInfo:[self errorUserInfo]];
                return NO;
            }
        } else if (_mp4Parser.videoCodecID == CommonQGMP4VideoStreamCodecIDH265) {
...
- (BOOL)createDecompressionSession {
    ...
    CFNumberRef value = CFNumberCreate(NULL, kCFNumberSInt32Type, &v);
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue >= 9.0) {
        ...
        if (_status != noErr) {
            CFRelease(attrs);
            _constructErr = [NSError errorWithDomain:CommonQGMP4HWDErrorDomain code:CommonQGMP4HWDErrorCode_ErrorCreateVTBSession userInfo:[self errorUserInfo]];
            return NO;
        }
    } else {
...
- (void)findKeyFrameAndDecodeToCurrent:(NSInteger)frameIndex {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kCQGVAPDecoderSeekStart object:self];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kCQGVAPDecoderSeekFinish object:self];
}
