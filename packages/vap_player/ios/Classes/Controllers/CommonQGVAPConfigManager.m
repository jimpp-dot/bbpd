...
#import "NSDictionary+VAPUtilTexture.h"
#import "UIColor+VAPUtilTexture.h"
#import "NSArray+VAPUtilTexture.h"
#import "CommonQGHWDMetalRenderer.h"
#import "CommonQGVAPTextureLoader.h"

@interface CommonQGVAPConfigManager () {
    
    CommonQGMP4HWDFileInfo *_fileInfo;
}

@end

@implementation CommonQGVAPConfigManager
...
- (void)setupConfig {
    ...
    CommonQGMP4Box *vapc = [_fileInfo.mp4Parser.rootBox subBoxOfType:CommonQGMP4BoxType_vapc];
    if (!vapc) {
...
- (void)loadConfigResources {
    ...
    if ([self.delegate respondsToSelector:@selector(vap_contentForTag:resource:)]) {
        [self.model.resources enumerateObjectsUsingBlock:^(CommonQGVAPSourceInfo * _Nonnull resource, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.model.resources enumerateObjectsUsingBlock:^(CommonQGVAPSourceInfo * _Nonnull resource, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([resource.type isEqualToString:kCQGAGAttachmentSourceTypeText] && [resource.loadType isEqualToString:kCQGAGAttachmentSourceLoadTypeLocal]) {
            resource.sourceImage = [CommonQGVAPTextureLoader drawingImageForText:tagContent color:resource.color size:resource.size bold:[resource.style isEqualToString:kCQGAGAttachmentSourceStyleBoldText]];
        }
        
        if ([resource.type isEqualToString:kCQGAGAttachmentSourceTypeImg] && [resource.loadType isEqualToString:kCQGAGAttachmentSourceLoadTypeNet]) {
...
- (void)loadMTLTextures:(id<MTLDevice>)device {
    ...
    [self.model.resources enumerateObjectsUsingBlock:^(CommonQGVAPSourceInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        id<MTLTexture> texture = [CommonQGVAPTextureLoader loadTextureWithImage:obj.sourceImage device:device];
        obj.sourceImage = nil;
        obj.texture = texture;
    }];
}
...
- (void)loadMTLBuffers:(id<MTLDevice>)device {
    ...
    [self.model.resources enumerateObjectsUsingBlock:^(CommonQGVAPSourceInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        id<MTLBuffer> buffer = [CommonQGVAPTextureLoader loadVapColorFillBufferWith:obj.color device:device];
...
- (void)parseConfigDictinary:(NSDictionary *)configDic {
    ...
    CommonQGVAPConfigModel *configModel = [CommonQGVAPConfigModel new];
    //parse
    ...
    //整体信息
    CommonQGVAPCommonInfo *commonInfo = [CommonQGVAPCommonInfo new];
    ...
    //源信息
    NSMutableDictionary <NSString *, CommonQGVAPSourceInfo *>*sources = [NSMutableDictionary new];
    [sourcesArr enumerateObjectsUsingBlock:^(NSDictionary *sourceDic, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        //parse
        CommonQGAGAttachmentSourceType sourceType = [sourceDic hwd_stringValue:@"srcType"];
        CommonQGAGAttachmentSourceLoadType loadType = [sourceDic hwd_stringValue:@"loadType"];
        ...
        CommonQGAGAttachmentSourceStyle style = [sourceDic hwd_stringValue:@"style"];
        CGFloat width = [sourceDic hwd_floatValue:@"w"];
        CGFloat height = [sourceDic hwd_floatValue:@"h"];
        CommonQGAGAttachmentFitType fitType = [sourceDic hwd_stringValue:@"fitType"];
        CommonQGVAPSourceInfo *sourceInfo = [CommonQGVAPSourceInfo new];
    ...
    NSMutableDictionary <NSNumber *, NSArray<CommonQGVAPMergedInfo *>*> *mergedConfig = [NSMutableDictionary new];
    [framesArr enumerateObjectsUsingBlock:^(NSDictionary *frameMergedDic, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        NSMutableArray <CommonQGVAPMergedInfo *> *mergedInfos = [NSMutableArray new];
        ...
        [mergedObjs enumerateObjectsUsingBlock:^(NSDictionary *mergeInfoDic, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            CommonQGVAPSourceInfo *sourceInfo = sources[sourceID];
            if (!sourceInfo) {
            ...
            CommonQGVAPMergedInfo *mergeInfo = [CommonQGVAPMergedInfo new];
        ...
        NSArray *sortedMergeInfos = [mergedInfos sortedArrayUsingComparator:^NSComparisonResult(CommonQGVAPMergedInfo *info1, CommonQGVAPMergedInfo *info2) {
