...
class TacitDialog extends StatefulWidget {
  ...
  const TacitDialog({required this.targetUid, super.key});
  ...
  static Future<void> show(BuildContext context, String targetUid) {
...
class TacitDialogState extends State<TacitDialog> {
  ...
  Future<void> loadData({bool change = false}) async {
    ResTacitQuestion res =
        await TacitRepo.getTacitQuestion(question?.question.id ?? 0);
    if (mounted) {
      if (!change) {
        setState(() {
          question = res;
        ...
        GestureDetector(
          onTap: () {
            ...
            Tracker.instance
              ...
              'questions_page_view_list_change': 1,
            });
          },
          behavior: HitTestBehavior.opaque,
          child: Container(
            ...
            alignment: AlignmentDirectional.center,
            child: Text(
              K.chat_tacit_dialog_change_button,
              style: const TextStyle(
                fontSize: 15,
        ...
        GestureDetector(
          onTap: () {
            ...
            if (select > 0) {
              ...
              Tracker.instance
                  .track(TrackEvent.questions_page_view_list, properties: {
                'questions_page_view_list_send': 1,
              });
            }
          },
          behavior: HitTestBehavior.opaque,
    ...
    return GestureDetector(
      onTap: () {
        if (select != index && mounted) {
          ...
          Tracker.instance
              .track(TrackEvent.questions_page_view_list, properties: {
            'questions_page_view_answer': index,
          });
        }
      },
      behavior: HitTestBehavior.opaque,
  ...
  void send(int select) async {
    ...
    isSend = true;
    ResSendTacitQuestion res = await TacitRepo.sendTacit(
        question?.question.id ?? 0, widget.targetUid, select);
    if (mounted) {
      if (res.success == true) {
        Navigator.pop(context);
      } else if (Util.validStr(res.msg)) {
