...
import 'package:profile/src/new_user_pay_gs_user_sub_list.dart';

class NewUserPayGsUserDetail extends StatefulWidget {
  const NewUserPayGsUserDetail({super.key});

  @override
  State<StatefulWidget> createState() => NewUserPayGsUserState();

  static Future<void> show(BuildContext context) {
    return Navigator.of(context).push(MaterialPageRoute(
      builder: (context) => const NewUserPayGsUserDetail(),
    ));
  }
}

class NewUserPayGsUserState extends State<NewUserPayGsUserDetail> {
  ...
  void initState() {
    ...
    _tabs.forEachIndexed((i, e) {
      _pagesWidget.add(NewUserPayGsUserSubList(dateTab: (i + 1)));
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: BaseAppBar.custom(
        title:
            Text(K.profile_new_user_pay_user_title, style: R.textStyle.title),
      ),
