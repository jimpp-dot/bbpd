...
import 'package:message/src/accost/widget/strategy_voice_play_widget.dart';
import 'package:message/src/model/accost_msg_item.dart';
import 'strategy_add_content_widget.dart';
import 'strategy_modify_buttons.dart';

class StrategyMsgItemWidget extends StatelessWidget {
  final int index;
  ...
  Widget _buildTypeContent(AccostMsgItem item) {
    bool auditFail = item.state == 2;
    if (item.isText) {
      return Container(
      ...
      return StrategyVoicePlayWidget(
        item: item,
      );
    } else if (item.isImage) {
      return ClipRRect(
      ...
      return StrategyAddContentWidget(
        item: item,
        onAddText: () => onTapText(),
