...
class BbAccostStrategySortPage extends StatefulWidget {
  final List<GsChatMsg> msgList;

  const BbAccostStrategySortPage({super.key, required this.msgList});

  static Future openBbAccostStrategySortPage(
      BuildContext context, List<GsChatMsg> msgList) {
    return Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) {
          return BbAccostStrategySortPage(
            msgList: msgList,
          );
        },
  ...
  @override
  _BbAccostStrategySortPageState createState() =>
      _BbAccostStrategySortPageState();
}

class _BbAccostStrategySortPageState extends State<BbAccostStrategySortPage> {
  ...
  void initState() {
    super.initState();
    for (int i = 0; i < widget.msgList.length; i++) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: BaseAppBar(
        ...
        backgroundColor: R.colors.homeBgColor,
        onPressed: () {
          Navigator.pop(context, widget.msgList);
        },
        ...
        Expanded(
          child: ReorderableListView.builder(
            ...
            onReorder: (oldIndex, newIndex) {
              ...
              var child = widget.msgList.removeAt(oldIndex);
              widget.msgList.insert(newIndex, child);
              setState(() {});
            },
          ),
        ),
  ...
  Widget _buildMsgItem(int index) {
    GsChatMsg item = widget.msgList[index];
    String typeText = '';
    if (item.isText) {
        ...
        children: [
          ...
          GestureDetector(
            onTap: () {
              if (index > 0) {
                setState(() {
                  widget.msgList.remove(item);
                  widget.msgList.insert(0, item);
                });
              }
            },
            child: Container(
