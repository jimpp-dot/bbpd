...
///
///
class WolfGuideEndDialog extends StatefulWidget {
  ...
  @override
  _WolfGuideEndDialogState createState() => _WolfGuideEndDialogState();

  static show(
      BuildContext context, WolfModel wolfModel, VoidCallback disappear) async {
    ...
    RoomPosition? selfPosition = wolfModel.data.positionForCurrentUser();
...
class _WolfGuideEndDialogState extends State<WolfGuideEndDialog> {
  @override
  void initState() {
    super.initState();
...
class _WoldEndRolesWidget extends StatelessWidget {
    ...
    if (endType == EndType.WolfWin) {
      ...
      for (var _ in allPositions) {
        WolfGuidePosition? wolfPositionData =
            _.positionExpendData as WolfGuidePosition?;
        ...
        Container(
          ...
          child: TimerCountDownWidget(
            ...
            onClickBlock: () {
              _disMiss(context);
              //发言点击埋点
              Tracker.instance.track(TrackEvent.wolf_ai_game_end_continue_click,
                  properties: {});
            },
          ),
        ),
  ...
  _disMiss(BuildContext context) async {
    if (ModalRoute.of(context)?.isCurrent == true) {
      Navigator.of(context).pop();
      if (disappear != null) {
        disappear!();
...
class _WoldEndRoleItemWidget extends StatelessWidget {
  ...
  _buildSexNameWidget() {
    return Padding(
      ...
      child: Row(
        children: <Widget>[
          if (roomPosition.sex == Sex.Male || roomPosition.sex == Sex.Female)
            Padding(
  ...
  _buildPositionWidget() {
    ...
    return Stack(
      children: <Widget>[
        ...
        Center(
          child: Text(
            isSelf ? K.wolf_self_position : '${roomPosition.position + 1}',
  ...
  String _getRoleImgPath(WolfGuidePosition wolfPositionData) {
      ...
      decoration: BoxDecoration(
        image: DecorationImage(
            image: AssetImage(R.imagePath(
                _getRoleImgPath(
                    roomPosition.positionExpendData as WolfGuidePosition),
                package: ComponentManager.MANAGER_WERE_WOLF))),
      ),
...
class TimerCountDownWidget extends StatefulWidget {
  Function onTimerFinishBlock;
  VoidCallback onClickBlock;
