...
class DaytimeHunterTakeAwayWidget extends StatelessWidget {
  final ChatRoomData room;

  const DaytimeHunterTakeAwayWidget({super.key, required this.room});
      ...
      children: <Widget>[
        Positioned(
          ...
          child: WolfSelectTargetActionWidget(
            actionTitle: K.wolf_v2_hunter_action_desc,
            buttonText: K.wolf_v2_hunter_action,
            onButtonTap: () async {
              if (targetPosition != null) {
                //没有选定目标不允许确认
                bool op = await wolfModel.gun(
                    room.rid, targetPosition.uidOrOrigin, true);
                if (op) {
                  //操作成功
                  wolfModel.actionUid = 0; //清除选中的目标
                }
                WolfSourceUtil.playVoice('wolf_day_time_hunter_gun_tips');
              }
            },
