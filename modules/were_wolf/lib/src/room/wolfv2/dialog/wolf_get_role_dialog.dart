...
class WolfGetRoleDialog extends StatefulWidget {
  final GetRoleData data;
  final WolfModel wolfModel;

  const WolfGetRoleDialog(
      {super.key, required this.data, required this.wolfModel});

  static show(BuildContext context,
      {Key? key,
      required GetRoleData data,
    ...
    await showGeneralDialog(
        ...
        pageBuilder: (BuildContext buildContext, Animation<double> animation,
          ...
          return WolfGetRoleDialog(key: key, data: data, wolfModel: wolfModel);
        });
  }
  ...
  @override
  _WolfGetRoleWidgetState createState() => _WolfGetRoleWidgetState();
}

class _WolfGetRoleWidgetState extends State<WolfGetRoleDialog> {
  RoleItem? _selectedItem;

  RoleItem? get selectedItem {
    RoomPosition? selfPosition = widget.wolfModel.room.positionForCurrentUser;
    if (selfPosition == null) return null;

    if (_selectedItem != null) return _selectedItem;

    if (widget.data.positionRoles.isEmpty || widget.data.roles.isEmpty)
      return null;

    RoleResultItem? roleResultItem;
    for (var _ in widget.data.positionRoles) {
      if (_.uid == selfPosition.uidOrOrigin) {
        roleResultItem = _;
        break;
      }
    }
    ...
    for (var _ in widget.data.roles) {
      if (_.roleKey == roleResultItem.roleKey) {
        _selectedItem = _;
        break;
      }
    }
  ...
  Widget _buildRoleItem(BuildContext context, RoleItem item) {
    ...
    return GestureDetector(
      onTap: () {
        RoomPosition? rp = WolfOpUtil.getRoomPosition(widget.wolfModel.room);
        if (rp == null) return;

        _selectedItem = item;

        widget.wolfModel
            .robRole(context, widget.wolfModel.room.rid, selectedItem!);
  ...
  Widget build(BuildContext context) {
    if (widget.data.roles.isEmpty) return const SizedBox();
    return WillPopScope(
        onWillPop: () async {
          return false;
        },
  ...
  Widget _buildPayBtn() {
    return GestureDetector(
        ...
        child: Container(
          ...
          child: Text(K.wolf_v2_wolf_role_jump,
  ...
  int getTimer(RoomPosition? selfPosition) {
    int timer = (selfPosition?.counter != null && selfPosition!.counter > 0
            ...
            : widget.wolfModel.room.config!.counter) -
        widget.wolfModel.room.timestamp;
    return timer;
  }
