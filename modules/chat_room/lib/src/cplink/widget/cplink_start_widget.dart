...
import 'package:chat_room/src/cplink/widget/cplink_relation_widget.dart';
...
import 'cplink_operate_panel.dart';

class CpLinkStartWidget extends StatefulWidget {
  final double circleSize;
  final bool displayEmote;
  final ChatRoomData room;

  const CpLinkStartWidget(
      {Key? key,
  ...
  @override
  _CpLinkStartWidgetState createState() => _CpLinkStartWidgetState();
}

class _CpLinkStartWidgetState extends State<CpLinkStartWidget>
  ...
  RelationshipData? _relationData;
  ...
  void initState() {
    ...
    widget.room.addListener(
        RoomConstant.Event_Cplink_Relation_Refresh, _onRelationRefresh);
  ...
  @override
  void didUpdateWidget(covariant CpLinkStartWidget oldWidget) {
  ...
  _loadRelations() async {
    DataRsp<RelationshipData>? data =
        await CpLinkRepo.relationList(widget.room.rid);
    if (data?.success == true && cpLinkData?.state == CpLinkState.hand) {
      _relationData = data?.data;
      _refresh();
    }
  }
  ...
  _onRelationRefresh(String type, dynamic data) {
  ...
  Widget _buildStepItem(String title, bool isActive) {
    return Container(
      padding:
          const EdgeInsetsDirectional.only(start: 6, end: 6, top: 2, bottom: 2),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        isHand
            ? _buildHandSuccess(showLinkRelation)
            : Stack(
                children: [
                  _buildCpLinkCircle(),
                  Positioned.fill(
                    child: Center(
                      child: Stack(
                        ...
                        children: [
                          ...
                          // _buildCircleBg(),
                          // R.img('cplink/cp_link_center_bg.png', width: 2*r, height: 2*r, package: ComponentManager.MANAGER_ROOM),
                          // if (!_stateChanging) Container(
                          //   width: 88, height: 88,
                          //   alignment: AlignmentDirectional.center,
                          //   decoration: BoxDecoration(
                          //     image: DecorationImage(image: AssetImage(R.imagePath('cplink/cp_link_center_heart.png'), package: ComponentManager.MANAGER_ROOM)),
                          //   ),
                          //   child: Text(_cpLinkState == CpLinkState.heart ? K.room_cplink_state_heart : K.room_cplink_state_wait, style: TextStyle(fontSize: 14, color: Color(0xFFFFE7EB), fontWeight: FontWeight.w600),),
                          // ),
                          _buildLink(),
  ...
  Widget _buildHandSuccess(bool showLinkRelation) {
    return Stack(
      alignment: AlignmentDirectional.topCenter,
      children: [
        Container(
        ...
        Column(
          ...
          children: [
            HandSuccessWidget(
                ...
                room: widget.room,
                displayEmote: widget.displayEmote),
            CpLinkRelationWidget(
              relationshipData: _relationData,
              checkedRelationIndex: _checkedRelationIndex,
              onCheckChanged: (index, selected) {
                // if (!CpLinkUtil.isHost(widget.room)) {
                //   return;
                // }
                var item = _relationData?.relationships[index];
                if (item != null && item.lightStatus == false) {
                ...
                if (selected) {
                  ...
                  _checkedRelationIndex = index;
                }
                Log.d(
                    "onCheckChanged and bool = $selected _checkedIndex = $_checkedRelationIndex");
                setState(() {});
              },
            ),
          ],
