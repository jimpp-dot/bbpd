...
import '../model/cplink_repo.dart';

class CpLinkDiyRelationStatusDialog extends StatefulWidget {
  final ChatRoomData room;
  final AuctionRelationTotalProgressItem data;

  const CpLinkDiyRelationStatusDialog(
      {super.key, required this.room, required this.data});

  @override
  State<StatefulWidget> createState() => CpLinkDiyRelationStatusState();
  ...
  static show(BuildContext context, ChatRoomData room,
    ...
    return displayModalBottomSheet(
      ...
      maxHeightRatio: 1.0,
      builder: (context) =>
          CpLinkDiyRelationStatusDialog(room: room, data: data),
    );
  }
}

class CpLinkDiyRelationStatusState
    extends State<CpLinkDiyRelationStatusDialog> {
      ...
      child: Stack(
        children: [
          Align(
            ...
            child: _buildAvatar(widget.room.positions[2].icon),
          ),
          Align(
            alignment: AlignmentDirectional.centerStart,
            child: _buildAvatar(widget.room.positions[5].icon),
          ),
        ],
      ),
    );
  ...
  void _onTapDiyCommit() async {
    ...
    if (diyText.isNotEmpty == true) {
      NormalNull res = await CpLinkRepo.commitDiyRelation(
          rid: widget.room.realRid, diyName: diyText);
      if (res.success) {
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        K.room_cplink_diy_task_content(
                            ['${widget.data.price}']),
                        style: TextStyle(
                          color: R.color.mainTextColor,
                          fontSize: 15,
