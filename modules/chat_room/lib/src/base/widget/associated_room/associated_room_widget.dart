...
import 'associated_room_repo.dart';

class AssociatedRoomWidget extends StatefulWidget {
  final int rid;
  final int mainRid;
  final int endTime;
  ...
  const AssociatedRoomWidget({
    super.key,
    required this.rid,
    required this.mainRid,
  ...
  @override
  State<AssociatedRoomWidget> createState() => _AssociatedRoomWidgetState();
}

class _AssociatedRoomWidgetState extends State<AssociatedRoomWidget> {
  ...
  @override
  void didUpdateWidget(AssociatedRoomWidget oldWidget) {
  ...
  void _updateRemainderTime() {
    _remainderTime = widget.endTime - DateTime.now().secondsSinceEpoch;
  }
  ...
  Widget build(BuildContext context) {
    if (_remainderTime < 0 && widget.mainRid <= 0)
  ...
  void _turn2Room(int toRid, {int fromRid = 0}) {
    ...
    if (fromRid == 0) {
      ...
      Tracker.instance.track(TrackEvent.click_big_room,
          properties: {'big_room': toRid, 'small_room': widget.rid});
    } else {
      // 跳转到关联子房间
      roomManager?.openChatRoomScreenShow(context, toRid, mainRid: fromRid);
      Tracker.instance.track(TrackEvent.click_small_room,
          properties: {'big_room': fromRid, 'small_room': toRid});
    }
  }
}
