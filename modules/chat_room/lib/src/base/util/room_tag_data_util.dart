...
/// 房间tag工具
class RoomTagDataUtil {
  /// 根据tagId获取标签信息,此方法修改时,需注意是否需要修改TagDataUtil的方法
  static Future<CommonTagData?> getCommonTagData(
    ...
    if (Util.validStr(property)) {
      /// 限定标签显示房间类型
      if ('vip' == property && !room.isPrivate) {
        return null;
      } else if ('business' == property && !room.isBusiness) {
        return null;
      }
    }

    int onMicShow = Util.parseInt(data[TagConfigDb.columnOnMicShow]);
    if (onMicShow == 1 && !room.isMic) {
      /// 限定麦上用户才展示
      return null;
    }
    ...
    if (Util.validStr(roomFactoryType)) {
      ...
      String factoryType = room.config?.originFactory ?? '';
      if (factoryType != roomFactoryType) {
        return null;
      }
    }
    ...
    if (Util.validStr(channel) &&
        (!Util.validStr(room.config?.settlementChannel) ||
            !channel.contains(room.config!.settlementChannel!))) {
      /// 限定结算频道才展示
      return null;
    }
    ...
    if (Util.validStr(whiteListRids) &&
        !whiteListRids.contains('${room.realRid}')) {
      /// 限定白名单rid才展示
      return null;
    }
    ...
    try {
      tag = CommonTagData(
        ...
        fontColor: Util.notNullStr(data[TagConfigDb.columnFontColor]),
        tagId: tagId,
