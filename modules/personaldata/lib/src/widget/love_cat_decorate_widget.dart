...
class LoveCatDecorateWidget {
  static List<Widget> iconList(List<HomeProfileDecorate_ImageText> imageText) {
    if (imageText.length < 2) {
      return [];
    }

    double imgW = 510, imgH = 1104;
    double containerW = Util.width, containerH = Util.height;
  ...
  static List<Widget> nameList(List<HomeProfileDecorate_ImageText> imageText) {
    if (imageText.length < 2) {
      return [];
    }
