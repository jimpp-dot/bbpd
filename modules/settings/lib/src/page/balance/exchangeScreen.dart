...
const PayDataAndroid = [
  //{'money': 1 * 100, 'real': 1 * 100},
  {'gold': 1 * 100, 'real': 1 * 100},
  {'gold': 6 * 100, 'real': 6 * 100},
  {'gold': 30 * 100, 'real': 30 * 100},
  {'gold': 50 * 100, 'real': 50 * 100},
  {'gold': 100 * 100, 'real': 100 * 100},
  {'gold': 300 * 100, 'real': 300 * 100},
  {'gold': 500 * 100, 'real': 500 * 100},
  {'gold': 800 * 100, 'real': 800 * 100},
  {'gold': 1000 * 100, 'real': 1000 * 100},
];
...
class ExchangeScreen extends StatefulWidget {
  ...
  const ExchangeScreen({super.key, this.refer});
  ...
  static Future show(BuildContext context, {String? refer}) {
    return Navigator.of(context).push(MaterialPageRoute(
      ...
      settings: const RouteSettings(name: '/exchange'),
    ));
  }
}
...
class _ExchangeScreenState extends State<ExchangeScreen>
  ...
  _onNavigatorChanged(String type, dynamic data) {
    Log.d("DEBUG: ExchangeScreen._onNavigatorChanged, $type =====> $data");
    if (data is String) {
      if (type == "Navigator.Page.Pop" && data == '/exchange') {
        Log.d('_onNavigatorChanged back to exchange');
        _load();
      }
    }
  }
  ...
  void _load() async {
    try {
      ...
      Log.d("ExchangeScreen, _totalMoney: $_totalMoney");
    } catch (e) {
      Log.d(e);
    }
  }
  ...
  void _goRecharge() async {
    // await RechargeScreen.show(context, refer: widget.refer ?? 'coin');
                ...
                child: RichText(
                  text: TextSpan(
                      ...
                      children: <TextSpan>[
                        ...
                        TextSpan(
                          text:
                              ' ${_payData[_currentIndex]['gold'].toStringAsFixed(0)} ',
                          style: TextStyle(
                              color: R.color.thirdBrightColor, fontSize: 14),
                        ),
                        TextSpan(
                          text: K.setting_statement_gold_coin,
                          style: TextStyle(
                            color: R.color.secondTextColor,
                            fontSize: 14,
                          ),
  ...
  List<Widget> _buildItems() {
    ...
    for (int i = 0; i < _payData.length; i++) {
      ...
      String money = item['gold'].toStringAsFixed(0); // 充值数额
              ...
              child: Row(
                ...
                children: <Widget>[
                  Padding(
                    ...
                    child: R.img(
                      Assets.settings$ic_gold_coin_png,
                      width: 21.0,
          ...
          Expanded(
            child: Container(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    Text(
                      '${MoneyConfig.basePrice(100)}${K.setting_exchange_rate_new}',
                      style: TextStyle(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: BaseAppBar(K.setting_exchange_title),
      body: _buildBody(),
