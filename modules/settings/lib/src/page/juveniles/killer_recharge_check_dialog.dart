...
class KillerRechargeCheckDialog extends StatelessWidget {
  const KillerRechargeCheckDialog({super.key});

  /// 充值前置实名认证
  /// true 可以充值， false 不可以
  static Future<bool> show(bool upLimit) async {
    if (!Session.idAuthenticated) {
      if (upLimit) {
        bool? check = await showDialog(
            ...
            builder: (context) {
              return const KillerRechargeCheckDialog();
            });
        if (check != null && check) {
      ...
      if (Session.isAdult == 1) {
        ...
        Fluttertoast.showCenter(msg: K.setting_juvenlie_cannot_recharge);
      }
    }
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                Padding(
                  ...
                  child: Text(
                    K.setting_juvenlie_recharge_tips,
                    style:
