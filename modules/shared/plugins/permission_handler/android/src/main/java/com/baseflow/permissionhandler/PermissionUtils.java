        ...
        switch (permission) {
            ...
            case PermissionConstants.PERMISSION_GROUP_STORAGE:
                if(Build.VERSION.SDK_INT >=33){
                    if (hasPermissionInManifest(context, permissionNames, Manifest.permission.READ_MEDIA_IMAGES))
                        permissionNames.add(Manifest.permission.READ_MEDIA_IMAGES);

                    if (hasPermissionInManifest(context, permissionNames, Manifest.permission.READ_MEDIA_VIDEO))
                        permissionNames.add(Manifest.permission.READ_MEDIA_VIDEO);

                    if (hasPermissionInManifest(context, permissionNames, Manifest.permission.READ_MEDIA_AUDIO)){
