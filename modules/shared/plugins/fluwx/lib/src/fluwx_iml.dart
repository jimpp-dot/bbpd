...
Future<bool> registerWxApi({
  ...
  return await _channel
      .invokeMethod('registerApp', {'appId': appId, 'iOS': doOnIOS, 'android': doOnAndroid, 'universalLink': universalLink});
}
...
Future<bool> sendWeChatAuth({required String scope, String state = 'state', bool nonAutomatic = false}) async {
...
Future<bool> launchWeChatMiniProgram({
  ...
  return await _channel
      .invokeMethod('launchMiniProgram', {'userName': username, 'path': path, 'miniProgramType': miniProgramType.toNativeInt()});
}
...
Future<bool> authWeChatByQRCode({
  ...
  return await _channel.invokeMethod('authByQRCode',
      {'appId': appId, 'scope': scope, 'nonceStr': nonceStr, 'timeStamp': timeStamp, 'signature': signature, 'schemeData': schemeData});
}
...
Future<bool> openWeChatInvoice(
    {required String appId, required String cardType, String locationId = "", String cardId = "", String canMultiSelect = "1"}) async {
