...
abstract class _CustomLayoutStateBase<T extends _SubSwiper> extends State<T>
  ...
  int _startIndex = 0;
  int? _animationCount;
  int _startIndexOffset = 0;

  /// 初始化startIndex的偏移量，建议配合CommonCustomLayoutSwiper组件使用
  ...
  void _move(double position, {int? nextIndex}) async {
    ...
    try {
      ...
      if (nextIndex != null) {
        ///
        int indexNeedsFix = nextIndex;
        if (_startIndexOffset > 0) {
          indexNeedsFix += (_startIndex + _startIndexOffset);
        }
        widget.onIndexChanged?.call(widget.getCorrectIndex(indexNeedsFix));
      }
    } catch (e) {
      Log.d(e);
    } finally {
...
Offset _getOffsetValue(List<Offset> values, double animationValue, int index) {
  // Log.d('_getOffsetValue index = $index');
  // Log.d('_getOffsetValue animationValue = $animationValue');
  ...
  if (animationValue >= 0.5) {
    ...
    if (index < values.length - 1) {
      ...
      dx = dx + (2.0 * values[index].dx - dx) * (animationValue - 0.5) * 2.0;
      dy = dy + (2.0 * values[index].dy - dy) * (animationValue - 0.5) * 2.0;
    }
  } else {
    /// 向左滚动
    if (index != 0) {
      ...
      /// code by liying 将自定义Layout效果更改为类似PageView的滚动效果
      dx = dx - (dx - 2.0 * values[index].dx) * (0.5 - animationValue) * 2.0;
      dy = dy - (dy - 2.0 * values[index].dy) * (0.5 - animationValue) * 2.0;
    }
  }
  // Log.d('_getOffsetValue = ${Offset(dx, dy).toString()}');
  return Offset(dx, dy);
}
...
class ScaleTransformBuilder extends TransformBuilder<double> {
  ...
  ScaleTransformBuilder(
      {List<double> values = const [], this.alignment = Alignment.center})
      : super(values: values);
...
class OpacityTransformBuilder extends TransformBuilder<double> {
  OpacityTransformBuilder({List<double> values = const []})
      : super(values: values);
...
class RotateTransformBuilder extends TransformBuilder<double> {
  RotateTransformBuilder({List<double> values = const []})
      : super(values: values);
...
class TranslateTransformBuilder extends TransformBuilder<Offset> {
  TranslateTransformBuilder({List<Offset> values = const []})
      : super(values: values);
...
class CustomLayoutOption {
  ...
  /// 初始index的偏移量,用来后面计算realIndex

  CustomLayoutOption(
      {this.stateCount = 0, this.startIndex = 0, this.startIndexOffset = 0});
...
class _CustomLayoutState extends _CustomLayoutStateBase<_CustomLayoutSwiper> {
  ...
  void didChangeDependencies() {
    ...
    _startIndexOffset = widget.option.startIndexOffset;
