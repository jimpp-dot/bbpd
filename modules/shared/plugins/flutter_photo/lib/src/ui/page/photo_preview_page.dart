...
class _PhotoPreviewPageState extends State<PhotoPreviewPage> {
  ...
  Widget build(BuildContext context) {
    int totalCount = 0; //FIXME: assetProvider.current.assetCount ?? 0;
    if (!widget.isPreview) {
      totalCount = 0; //FIXME: assetProvider.current.assetCount ?? 0;
    } else {
      ...
      child: StreamBuilder<int>(
        builder: (ctx, AsyncSnapshot<int> snapshot) {
          ...
          if (list.isNotEmpty && index >= 0 && index < list.length) {
            var data = list[index];
            checked = selectedList.contains(data);
          }
  ...
  void _onChangeProvider(bool check, int index) {
    var item = list[index];
    if (check) {
      selectedProvider.addSelectEntity(item, options.selectInterceptor);
    } else {
