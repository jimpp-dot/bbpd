...
import static ly.plugins.shared_app_plugin.utils.CheckResult.RESULT_EMULATOR;
import static ly.plugins.shared_app_plugin.utils.CheckResult.RESULT_MAYBE_EMULATOR;
import static ly.plugins.shared_app_plugin.utils.CheckResult.RESULT_UNKNOWN;
...
//                return true;
        ...
        //检测传感器数量
        int sensorNumber = 1; // getSensorNumber(context); 安全审查不允许读取
        if (sensorNumber <= 7) ++suspectCount;

        //检测已安装第三方应用数量
        // 整改：涉及用户隐私，不再获取应用安装列表
...
//                    .append("\r\n").append("userAppNumber = ").append(userAppNumber)
            ...
            properties.put("hardware", hardwareResult.value);
            properties.put("flavor", flavorResult.value);
            properties.put("model", modelResult.value);
            properties.put("manufacturer", manufacturerResult.value);
            properties.put("board", boardResult.value);
            properties.put("platform", platformResult.value);
            properties.put("baseBand", baseBandResult.value);
            properties.put("sensorNumber", sensorNumber);
//            properties.put("userAppNumber", userAppNumber);
            properties.put("supportCamera", supportCamera);
            properties.put("supportCameraFlash", supportCameraFlash);
            properties.put("supportBluetooth", supportBluetooth);
            properties.put("hasLightSensor", hasLightSensor);
            properties.put("cgroupResult", cgroupResult.value);
            callback.findEmulator(stringBuffer.toString(), properties);
        }
