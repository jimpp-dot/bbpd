...
NSString *const APPDATA = @"appData";
...
bool launchAppFromRemoteNotification;
...
- (void)initialize:(FlutterMethodCall * _Nonnull)call result:(FlutterResult _Nonnull)result {
    ...
    if(@available(iOS 10.0, *)) {
        ...
        if(launchNotification != nil) {
            NSString *payload = [self handlePayloadStrWithUserInfo:launchNotification.userInfo];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    if([INITIALIZE_METHOD isEqualToString:call.method]) {
        ...
        NSInteger badgeNumber = [call.arguments integerValue];
        [[UIApplication sharedApplication] setApplicationIconBadgeNumber:badgeNumber];
        result(nil);
    }else if([@"getBadgeNumber" isEqualToString:call.method]){
        ...
        NSString *payload;
        if(launchNotification != nil) {
            payload = [self handlePayloadStrWithUserInfo:launchNotification.userInfo];
        } else {
...
didReceiveNotificationResponse:(UNNotificationResponse *)response
    ...
    if (launchAppFromRemoteNotification) {
        launchAppFromRemoteNotification = false;
        return;;
    }
    if ([response.actionIdentifier isEqualToString:UNNotificationDefaultActionIdentifier]) {
        NSString *payload = [self handlePayloadStrWithUserInfo:response.notification.request.content.userInfo];
        if(initialized) {
...
didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if (remoteDict) {
        launchAppFromRemoteNotification = true;
    }
    return YES;
}
...
didReceiveLocalNotification:(UILocalNotification*)notification {
    NSDictionary *arguments = [NSDictionary dictionaryWithObjectsAndKeys:notification.userInfo[NOTIFICATION_ID], ID,notification.userInfo[TITLE], TITLE,notification.alertBody, BODY,  [self handlePayloadStrWithUserInfo:notification.userInfo], PAYLOAD, nil];
    [channel invokeMethod:DID_RECEIVE_LOCAL_NOTIFICATION arguments:arguments];
}

- (NSString *)handlePayloadStrWithUserInfo:(NSDictionary *)userInfo{
    NSString *payloadStr = userInfo[PAYLOAD];
    if (payloadStr != nil && payloadStr.length > 0) {
        return payloadStr;
    };
    NSString *appDataStr = userInfo[APPDATA];
    if (appDataStr != nil && appDataStr.length > 0) {
        return [NSString stringWithFormat:@"rong://%@/conversationlist?isFromPush=true&%@",
                [[NSBundle mainBundle] bundleIdentifier], payloadStr];
    }
    return @"";
}
