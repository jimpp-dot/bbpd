...
abstract class SliverWaterfallFlowDelegate extends ExtendedListDelegate {
  ...
  const SliverWaterfallFlowDelegate({
    ...
    double exposureRatioThreshold = 0.0,
        ...
        super(
          ...
          exposureRatioThreshold: exposureRatioThreshold,
        );
...
class RenderSliverWaterfallFlow extends RenderSliverMultiBoxAdaptor
  ...
  void performLayout() {
      ...
      if (!advance()) {
        ...
        if (_previousCrossAxisChildrenData != null) {
          final int minTrailingIndex =
              _previousCrossAxisChildrenData!.minTrailingIndex ?? 0;
    ...
    //zmt
    callViewportBuilder(
        viewportBuilder: _gridDelegate.viewportBuilder,
        exposureRatioThreshold: _gridDelegate.exposureRatioThreshold);
...
class _CrossAxisChildrenData {
  ...
  void insertLeading({
    ...
    if (!leadingChildren.contains(data)) {
      ...
      // begin：waterfall_flow插件异常处理
      if (!child.hasSize) {
        print('child has not size');
      }
      data.layoutOffset = data.trailingLayoutOffset -
          (child.hasSize ? paintExtentOf(child) : 0);
      // end：waterfall_flow插件异常处理
