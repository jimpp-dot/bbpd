...
class ExtendedListDelegate {
  const ExtendedListDelegate({
    ...
    this.exposureRatioThreshold = 0.0,
  });
  ...
  /// for viewportBuilder.
  /// If item exposure ratio exceed this value, viewportBuilder will count.
  final double exposureRatioThreshold;
}
...
mixin ExtendedRenderObjectMixin on RenderSliverMultiBoxAdaptor {
  ...
  void callViewportBuilder({
    ...
    double exposureRatioThreshold = 0,
  }) {
    ...
    while (viewportLastChild != null) {
      final double layoutOffset = childScrollOffset(viewportLastChild);
      final double lastChildHeight = viewportLastChild.size.height;
      final double trailingOffset = layoutOffset +
          (getPaintExtend != null
              ? getPaintExtend(viewportLastChild)
              : paintExtentOf(viewportLastChild));
      if (layoutOffset + lastChildHeight * exposureRatioThreshold <
