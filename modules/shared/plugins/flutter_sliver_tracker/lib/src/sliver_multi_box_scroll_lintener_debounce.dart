...
class _State extends State<SliverMultiBoxScrollListenerDebounce>
  ...
  late Function _onScrollUpdateDebounce;
  ...
  void initState() {
    ...
    _onScrollUpdateDebounce = debounce(() {
      if (currentPaintPercent >= widget.minPaintPercent) {
        ...
        if (widget.notifyOnce) hasNotified = true;
      }
    }, widget.debounce);
  }
  ...
  void _onScrollUpdate(ScrollUpdateNotification notification, double itemLength,
  ...
  void _onScrollEnd(ScrollEndNotification notification, double itemLength,
      double displayedLength) {
    // 更新展示比例
    currentPaintPercent = displayedLength / itemLength;
