...
class ScrollViewListener extends StatefulWidget {
  ...
  const ScrollViewListener(
      {Key? key, required this.child, this.onScrollEnd, this.onScrollUpdate})
      : super(key: key);
  ...
  static Stream<ScrollNotification>? of(BuildContext context) {
    return context
        .findAncestorStateOfType<ScrollViewListenerState>()
        ?.broadCaseStream;
  }
}

class ScrollViewListenerState extends State<ScrollViewListener> {
  late StreamController<ScrollNotification> controller;
  late Stream<ScrollNotification> broadCaseStream;
  // 目标ScrollableState
      ...
      if (mounted) {
        ...
        while (childEle != null) {
          if (childEle!.widget is Scrollable) {
            _scrollableState =
                (childEle as StatefulElement).state as ScrollableState;
            break;
          } else {
