...
class ScaleGestureRecognizer extends OneSequenceGestureRecognizer {
  ...
  ScaleGestureRecognizer({super.debugOwner});
  ...
  bool isWithinTolerance(PointerEvent event, double tolerance) {
    final Offset offset = event.position - (_initialFocalPoint ?? Offset.zero);
      ...
      if (pointCount <= 1 &&
        ...
        invokeCallback<void>(
            ...
            () => onUpdate!(ScaleUpdateDetails(
                scale: _scaleFactor,
                focalPoint: _currentFocalPoint ?? Offset.zero,
                rotation: _computeRotationFactor(),
                pointerEvent: event,
                pointCount: pointCount)));
      }
    } else if (event is PointerDownEvent) {
      isOnlyOnePoint = _pointerQueue.isEmpty;
  ...
  void _update() {
    ...
    for (int pointer in _pointerLocations.keys) {
      totalDeviation +=
          ((_currentFocalPoint ?? Offset.zero) - _pointerLocations[pointer]!)
              .distance;
    }
    ...
    if (count < 2) {
      ...
      _currentLine = _LineBetweenPointers(
          ...
          pointerStartLocation:
              _pointerLocations[_pointerQueue[0]] ?? Offset.zero,
          pointerEndId: _pointerQueue[1],
          pointerEndLocation:
              _pointerLocations[_pointerQueue[1]] ?? Offset.zero);
    } else {
  ...
  void _advanceStateMachine(
    ...
    if (_state == _ScaleState.possible) {
      ...
      final double focalPointDelta = ((_currentFocalPoint ?? Offset.zero) -
