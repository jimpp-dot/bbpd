...
const List<String> _kBlockTags = <String>[
  ...
  'tbody',
  'tr',
  'Br', //br已被占用  //Br 标签的实际效果为Container(height:brHeight+blockSpacing),
  'video',
  'audio'
];
  ...
  void visitElementAfter(md.Element element) {
    ...
    if (_isBlockTag(tag)) {
      ...
      if (_isListTag(tag)) {
        ...
        child = Container(
//          color: Colors.red,
          height: styleSheet?.brHeight,
        );
      } else if (tag == 'video') {
        dynamic video = element.children![0];
        dynamic img = element.children![1];
          ...
          children: <Widget>[
            ...
            Center(
              child: SvgPicture.asset(
                'assets/ic_video_play.svg',
                package: 'flutter_markdown',
                width: 36,
                height: 36,
              ),
            ),
            // Positioned.fill(child: delegate.createVideoGesturer(video.text, img.text))
        ...
        dynamic audioUrl = element.children![0];
        dynamic audioDuration = element.children![1];
        child = AudioButton(
          audioUrl: audioUrl.text,
          duration: int.parse(audioDuration.text),
          isLocal: false,
      ...
      if (tag == 'img') {
        ...
        if (style == null) {
          align = tag == 'th' ? styleSheet?.tableHeadAlign : TextAlign.start;
        } else {
  ...
  List<Widget> _mergeInlineChildren(List<Widget> children) {
    ...
    for (Widget child in children) {
      if (mergedTexts.isNotEmpty &&
        ...
        List<InlineSpan> children = previousTextSpan.children != null
        ...
        TextSpan previousTextSpan = previous.textSpan as TextSpan;
        List<InlineSpan> children = previousTextSpan.children != null
            ? List.from(previousTextSpan.children!)
            : [previousTextSpan];
  ...
  Widget _buildRichText(TextSpan text) {
    if (selectable) {
      ...
      return RichText(
        text: text,
        textScaler: TextScaler.linear(styleSheet!.textScaleFactor),
      );
    }
  }
}
