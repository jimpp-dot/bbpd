...
class DragLike extends StatefulWidget {
  ...
  final VoidCallback? onRunBackComplete;
  const DragLike(
      {Key? key,
      required this.child,
...
class _DragLikeState extends State<DragLike> with TickerProviderStateMixin {
  ...
  void updatePosition(positionX, {iscontroller = false}) {
    // Log.d("positionX = " + positionX.toString());
    // Log.d("lastPositionX = " + lastPositionX.toString());
    ...
    // Log.d("offsetAbs = " + offsetAbs.toString());
    double angleTemp =
    ...
    if (angle.value != angleTemp) {
      ...
      // Log.d("dragDistance = " + dragDistance.toString());
    ...
    animation = Tween<double>(begin: angle.value, end: 0).animate(controller)
      ..addListener(() {
        ...
        if (controller.isCompleted) {
          controller.dispose();
          if (widget.onRunBackComplete != null) {
            widget.onRunBackComplete!();
          }
        }
        // setState(() {});
      });
  ...
  void runOutAnimate(int type,
    ...
    scaleComplete = false;
    // 回调给上层
    widget.onOutComplete(type == 1
        ? (completeTag == '' ? 'right' : completeTag)
        : (completeTag == '' ? 'left' : completeTag));
...
class DragController {
  ...
  void controlerInit(_DragLikeState widgetState) {
