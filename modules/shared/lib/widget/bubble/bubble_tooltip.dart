...
/// other user interface action. Wrap the button in a [BubbleTooltip] widget to
...
/// widget to include a [BubbleTooltip] in its build.
///
...
class BubbleTooltip extends StatefulWidget {
  ...
  /// [BubbleTooltip.message]. Set this property to true if the app is going to
...
//  final Decoration decoration;
  ...
  @override
  BubbleTooltipState createState() => BubbleTooltipState();
...
class BubbleTooltipState extends State<BubbleTooltip>
  ...
  static const Duration _fadeOutDuration = Duration(milliseconds: 200);
  static const Duration _defaultShowDuration = Duration(milliseconds: 3000);
  static const Duration _defaultWaitDuration = Duration(milliseconds: 0);
  static const bool _defaultExcludeFromSemantics = false;

  double height = _defaultTooltipHeight;
  EdgeInsetsGeometry? padding;
  EdgeInsetsGeometry? margin;
  Decoration? decoration;
  TextStyle? textStyle;
  double verticalOffset = _defaultVerticalOffset;
  bool preferBelow = _defaultPreferBelow;
  bool excludeFromSemantics = _defaultExcludeFromSemantics;
  late AnimationController _controller;
  OverlayEntry? _entry;
  Timer? _hideTimer;
  Timer? _showTimer;
  Duration showDuration = _defaultShowDuration;
  Duration waitDuration = _defaultWaitDuration;
  ...
  void _showTooltipInternal({bool immediately = false}) {
    _hideTimer?.cancel();
    _hideTimer = null;
    if (immediately) {
  ...
  void _createNewEntry() {
    ...
    Offset target = box.localToGlobal(box.size.bottomCenter(Offset.zero));
    // 去掉展示内容的内部paddingBottom
    target = target - Offset(0, widget.childPaddingBottom);
  ...
  void showTooltip() {
    _longPressActivated = true;
    _showTooltipInternal(immediately: true);
    _hideTooltip();
  }
...
//    decoration = widget.decoration ?? tooltipTheme.decoration ?? defaultDecoration;
    ...
    if (_mouseIsConnected) {
      result = MouseRegion(
        onEnter: (PointerEnterEvent event) => _showTooltipInternal(),
...
class _TooltipPositionDelegate extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    Log.d(
        'GetPositionForChild, size:$size, childSize:$childSize, offset:$offset, target:$target');
    return offset;
  }
        ...
        child: FadeTransition(
            ...
            child: CustomSingleChildLayout(
              ...
              child: CustomPaint(
                key: customPaintKey,
                foregroundPainter:
                    BubbleArrow(target, customPaintKey, verticalOffset),
...
class BubbleArrow extends CustomPainter {
  Offset target;
  GlobalKey context;
  double verticalDistance;
  List<Point<double>>? _points;

  BubbleArrow(this.target, this.context, this.verticalDistance);

  double get angle {
    return pi / 4;
  }

  Offset get targetToLocal {
    final RenderBox box =
        context.currentContext?.findRenderObject() as RenderBox;
    final Offset localTarget = box.localToGlobal(box.size.topLeft(Offset.zero));

    Log.d('In get targetX, target: $target, localTarget: $localTarget');
    return Offset(target.dx - localTarget.dx, target.dy - localTarget.dy);
  }

  List<Point<double>> get points {
    if (_points == null) {
      double dx = tan(angle) * verticalDistance * 2 / 3;
      _points = [
        Point(targetToLocal.dx, targetToLocal.dy + verticalDistance / 3),
        Point(targetToLocal.dx + dx, targetToLocal.dy + verticalDistance),
        Point(targetToLocal.dx - dx, targetToLocal.dy + verticalDistance),
      ];
    }
    return _points!;
  ...
  void paint(Canvas canvas, Size size) {
    Paint myPaint = Paint();
    myPaint.color = R.color.secondBgColor;
