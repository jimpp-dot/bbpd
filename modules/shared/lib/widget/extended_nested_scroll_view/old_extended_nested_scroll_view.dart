...
typedef CommonNestedScrollViewPinnedHeaderSliverHeightBuilder = double
    Function();
...
/// )
/// ```
class CommonNestedScrollView extends StatefulWidget {
  ...
  final CommonNestedScrollViewPinnedHeaderSliverHeightBuilder?
...
class NestedScrollViewState extends State<CommonNestedScrollView> {
  ...
  void initState() {
    ...
    _coordinator = _NestedScrollCoordinator(
        ...
        forceScroll: widget.forceScroll);
  }
  ...
  @override
  void didUpdateWidget(CommonNestedScrollView oldWidget) {
  ...
  void forceUpdateCanDrag(bool canDrag) {
    _coordinator.forceUpdateCanDrag(canDrag);
  }
...
class _NestedScrollCoordinator
  ...
  ScrollActivity createOuterBallisticScrollActivity(double velocity) {
    ...
    final _NestedScrollMetrics metrics =
        _getMetrics(innerPosition, velocity, isOut: true);
  ...
  _NestedScrollMetrics _getMetrics(
    ...
    // 整个view从上往下滑到顶，立即上滑时滑不动。
    // 由于往下滑到底inner会有个回弹效果，innerPosition.pixels为负值导致上滑时滑不动
    // 暂时强制当这种场景的innerPosition.pixels为0来处理滑动事件
    double innerPixels =
        (innerPosition.pixels > 0 || velocity < 0) ? innerPosition.pixels : 0;

    if (innerPixels == innerPosition.minScrollExtent) {
      ...
      assert(innerPixels != innerPosition.minScrollExtent);
      if (innerPixels < innerPosition.minScrollExtent) {
        pixels = innerPixels -
      ...
      if ((velocity > 0.0) && (innerPixels > innerPosition.minScrollExtent)) {
        ...
        correctionOffset = _outerPosition!.pixels - pixels;
          ...
          (innerPixels < innerPosition.minScrollExtent)) {
        ...
        minRange = _outerPosition!.minScrollExtent;
        if (isOut && velocity > 0) {
          // inner list下拉刷新不松手（outer已经滑上一部分）往上滑，outer回弹到初始状态BUG
          maxRange = minRange;
        } else {
  ...
  double nestOffset(double value, _NestedScrollPosition target, {velocity}) {
    ...
    if (value > _outerPosition!.maxScrollExtent) {
      if ((_outerPosition!.pixels + 0.001 < _outerPosition!.maxScrollExtent) &&
          velocity != null &&
          velocity > 0) {
        // 列表向上滑动时，outer 没有滑完，inner不开始滑
  ...
  void updateCanDrag() {
    if (_outerPosition == null || !_outerPosition!.haveDimensions) {
      return;
    }
    ...
    if (_innerPositions.isEmpty) {
      ...
      if (outerDelta != 0.0) {
        final double innerDelta =
            _outerPosition?.applyClampedPointerSignalUpdate(outerDelta) ?? 0;
        if (innerDelta != 0.0) {
          for (final _NestedScrollPosition position in _currentInnerPositions) {
      ...
      if (_floatHeaderSlivers)
        innerDelta =
            _outerPosition?.applyClampedPointerSignalUpdate(delta) ?? 0;
  ...
  Drag drag(DragStartDetails details, VoidCallback dragCancelCallback) {
    if (!_canDrag) {
      FakeDragController fakeDrag = FakeDragController(
          FakeScrollActivityDelegate(this, _outerPosition!),
          details,
          dragCancelCallback);
      _currentDrag = fakeDrag;
      beginActivity(_createIdleScrollActivity(_outerPosition!),
          _createIdleScrollActivity);
      return fakeDrag;
    }
  ...
  void applyUserOffset(double delta) {
    ...
    if (_innerPositions.isEmpty) {
      ...
      if (outerDelta != 0.0) {
        final double innerDelta =
            _outerPosition?.applyClampedDragUpdate(outerDelta) ?? 0;
        if (innerDelta != 0.0) {
          for (final _NestedScrollPosition position in _currentInnerPositions) {
      ...
      // Apply delta to the outer header first if it is configured to float.
      if (_floatHeaderSlivers) {
        innerDelta = _outerPosition?.applyClampedDragUpdate(delta) ?? 0;
      }
...
class _NestedScrollController extends ScrollController {
  ...
  Iterable<_NestedScrollPosition> getCurrentNestedPositions(
    ...
    if (innerScrollPositionKeyBuilder != null &&
        (scrollPositionKeyMap?.length ?? 0) > 1) {
      final Key key = innerScrollPositionKeyBuilder();
      if (scrollPositionKeyMap?.containsKey(key) == true) {
...
//    }
  ...
  void attachScrollPositionKey(_NestedScrollPosition position) {
    ...
    if (key != null) {
      if (scrollPositionKeyMap != null &&
          !scrollPositionKeyMap!.containsKey(key)) {
  ...
  void detachScrollPositionKey(_NestedScrollPosition position) {
    if (position.key != null &&
        scrollPositionKeyMap?.containsKey(position.key) == true) {
...
//    if (debugLabel == 'inner') {
//      Log.d("applyClampedDragUpdate$key");
//    }
...
//    }
    ...
    switch (mode) {
      case _NestedBallisticScrollActivityMode.outer:
        ...
        if (metrics?.minRange == metrics?.maxRange)
          return IdleScrollActivity(this);
        return _NestedOuterBallisticScrollActivity(coordinator, this, metrics!,
            simulation, context.vsync, activity?.shouldIgnorePointer ?? true);
      case _NestedBallisticScrollActivityMode.inner:
        return _NestedInnerBallisticScrollActivity(coordinator, this,
            simulation, context.vsync, activity?.shouldIgnorePointer ?? true);
  ...
  void updateCanDrag(double totalExtent) {
    ...
    if (forceScroll) context.setCanDrag(true);
  }
...
class _NestedInnerBallisticScrollActivity extends BallisticScrollActivity {
  ...
  bool applyMoveTo(double value) {
    return super.applyMoveTo(
        coordinator.nestOffset(value, delegate, velocity: velocity));
  }
}
...
class FakeDragController extends ScrollDragController implements Drag {
  FakeDragController(ScrollActivityDelegate delegate, DragStartDetails details,
...
class FakeScrollActivityDelegate extends ScrollActivityDelegate {
  final _NestedScrollCoordinator coordinator;
  final _NestedScrollPosition _outerPosition;

  FakeScrollActivityDelegate(this.coordinator, this._outerPosition);
  ...
  void goIdle() {
    coordinator.beginActivity(
        _NestedScrollCoordinator._createIdleScrollActivity(_outerPosition),
        _NestedScrollCoordinator._createIdleScrollActivity);
  }
