...
class LruCache<K, V> implements Map<K, V> {
  ...
  /// 缓存容器
  final Map<K, _Entry<K, V>> _cache = {};

  LruCache(this.maxSize, {this.head, this.tail}) : assert(maxSize > 0);

  /// map转为LruCache
  factory LruCache.of(Map<K, V> map, int maxSize) {
    LruCache<K, V> result = LruCache<K, V>(maxSize);
    result.addAll(map);
    return result;
  }
  ...
  void removeWhere(bool Function(K key, V value) predicate) {
    _cache.removeWhere((K key, _Entry<K, V> value) {
      if (value.value != null && predicate(key, value.value as V)) {
        _afterNodeRemoval(value);
        return true;
      }
      return false;
    });
  ...
  Iterable<MapEntry<K, V>> get entries {
    ...
    for (_Entry<K, V>? e = head; e != null; e = e.after) {
      if (e.key != null && e.value != null) {
        list.add(MapEntry(e.key as K, e.value as V));
      }
    }
    return list;
  }
  ...
  void operator []=(K key, dynamic value) {
    ...
    if (node == null) {
      ...
      _cache[key] = _createNew(key, value) as _Entry<K, V>;
    } else {
      _afterNodeAccess(node);
    }
  }
  ...
  V update(K key, V Function(V value) update, {V Function()? ifAbsent}) {
    updateFunc(_Entry<K, V> value) {
      V newValue = update(value.value as V);
      value.value = newValue;
      _afterNodeAccess(value);
      return value;
    }
  ...
  void updateAll(V Function(K key, V value) update) {
    _cache.updateAll((K key, _Entry<K, V> value) {
      V newValue = update(key, value.value as V);
      value.value = newValue;
      return value;
    });
  }
