import 'package:shared/assets.dart';
import 'package:shared/shared.dart';
import 'package:flutter/material.dart';
import 'package:gift/src/model/super_pack_beans.dart';
import 'package:gift/src/super_pack/super_pack_screen.dart';
import 'package:gift/src/widget/horizontal_price_selector.dart';
import 'package:gift/src/widget/super_pack_gift_item.dart';
import 'package:shared/k.dart' as BaseK;

import '../../assets.dart';
import '../../k.dart';

/// 新用户首次打开礼物面板时，弹出礼包弹窗
class SuperPackDialog extends StatefulWidget {
  static const SHOW_KEY = 'SuperPackDialog_Show';

  final SuperPackList? superPackList;

  const SuperPackDialog({Key? key, this.superPackList}) : super(key: key);

  @override
  _SuperPackDialogState createState() => _SuperPackDialogState();

  static Future<bool?> show(BuildContext context, SuperPackList superPackList) {
    return showDialog(
        context: System.context,
        barrierDismissible: true,
        builder: (context) => Center(
              child: SuperPackDialog(
                superPackList: superPackList,
              ),
            ));
  }

  static String getShowKey() {
    return '${SHOW_KEY}_${Session.uid}';
  }
}

class _SuperPackDialogState extends State<SuperPackDialog> {
  final List<String> _priceTags = [];

  int _selectedIndex = 0;

  double _realRadio = 90 / 160;

  @override
  void initState() {
    super.initState();
    widget.superPackList?.packages.forEach((item) {
      _priceTags.add('${_getPrice(item.recharge)}${BaseK.K.base_yuan}');
    });
    _selectedIndex = 0;

    Config.set(SuperPackDialog.getShowKey(), '1');
  }

  @override
  Widget build(BuildContext context) {
    return SizedBox(
      width: Util.width,
      child: Stack(
        children: [
          Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              SizedBox(
                width: Util.width - 40,
                child: Stack(
                  children: [
                    PositionedDirectional(
                        end: 0,
                        child: R.img(
                          Assets.gift_pack_gift_pack_dialog_header_webp,
                          width: 164,
                          height: 74,
                          package: ComponentManager.MANAGER_GIFT,
                        )),
                    _buildGiftPack(),
                  ],
                ),
              ),
              SizedBox(
                height: 12,
                width: Util.width,
              ),
              _buildClose()
            ],
          ),
          PositionedDirectional(
              bottom: 40,
              end: 0,
              child: R.img(
                Assets.gift_pack_gift_pack_dialog_footer_webp,
                width: 58,
                height: 64,
                package: ComponentManager.MANAGER_GIFT,
                fit: BoxFit.cover,
              )),
          PositionedDirectional(
              top: 20,
              start: 8,
              child: R.img(
                Assets.gift_pack_gift_pack_top_red_pkg_webp,
                width: 86,
                height: 74,
                package: ComponentManager.MANAGER_GIFT,
                fit: BoxFit.contain,
              ))
        ],
      ),
    );
  }

  Widget _buildGiftPack() {
    return Container(
      margin: const EdgeInsetsDirectional.only(top: 60),
      decoration: BoxDecoration(
        borderRadius: const BorderRadius.all(Radius.circular(16.0)),
        image: DecorationImage(
          image: AssetImage(
            R.imagePath(Assets.gift_pack_gift_pack_main_bg_webp,
                package: ComponentManager.MANAGER_GIFT),
          ),
          fit: BoxFit.fill,
        ),
      ),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Padding(
              padding:
                  const EdgeInsetsDirectional.only(start: 12, end: 12, top: 10),
              child: R.img(
                Assets.gift_pack_gift_pack_dialog_title_webp,
                width: Util.width,
                height: 52,
                package: ComponentManager.MANAGER_GIFT,
              )),
          HorizontalPriceSelector(
            tags: _priceTags,
            selectedIndex: _selectedIndex,
            onPriceSelected: _onPriceSelected,
          ),
          Padding(
            padding: const EdgeInsetsDirectional.only(
                start: 12, end: 12, bottom: 12),
            child: Container(
              padding: const EdgeInsetsDirectional.only(
                  start: 12, end: 12, top: 8, bottom: 8),
              decoration: BoxDecoration(
                color: Colors.white,
                borderRadius: BorderRadius.circular(15),
              ),
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  _buildItems(),
                  const SizedBox(
                    height: 12,
                  ),
                  _buildBottomBtn(),
                  const SizedBox(
                    height: 8,
                  ),
                ],
              ),
            ),
          )
        ],
      ),
    );
  }

  void _onPriceSelected(int index) {
    if (_selectedIndex != index) {
      _selectedIndex = index;
      setState(() {});
    }
  }

  Widget _buildItems() {
    SuperPackItem? superPackItem =
        widget.superPackList?.packages[_selectedIndex];

    List<Widget> children = [];
    if (superPackItem?.gifts.isNotEmpty == true) {
      for (int i = 0; i < superPackItem!.gifts.length; i++) {
        children.add(SuperPackGiftItem(
          data: superPackItem.gifts[i],
          size: 90,
          nameFontSize: 13,
          typeFontSize: 11,
          topTagWidth: 40,
          topTagHeight: 18,
          topTagFontSize: 11,
          onItemLayout: _onItemLayout,
        ));
      }
    }

    return Container(
      constraints: BoxConstraints(maxHeight: Util.height * 0.43),
      child: GridView.count(
        key: GlobalKey(),
        padding: const EdgeInsetsDirectional.only(
            start: 0, end: 0, bottom: 0, top: 0),
        scrollDirection: Axis.vertical,
        shrinkWrap: true,
        crossAxisCount: 3,
        mainAxisSpacing: 8.0,
        crossAxisSpacing: 8.0,
        childAspectRatio: _realRadio,
        children: children,
      ),
    );
  }

  Widget _buildBottomBtn() {
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: () {
        Navigator.of(context).pop();
        SuperPackScreen.show(Constant.context);
      },
      child: Container(
        width: 220,
        height: 48,
        decoration: const ShapeDecoration(
          shape: StadiumBorder(),
          gradient:
              LinearGradient(colors: [Color(0xFFFF4E4E), Color(0xFFFFA879)]),
        ),
        child: Center(
          child: Text(
            K.gift_buy_right_now,
            style: const TextStyle(
                color: Colors.white, fontSize: 16, fontWeight: FontWeight.w500),
          ),
        ),
      ),
    );
  }

  Widget _buildClose() {
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: () {
        Navigator.of(context).pop();
      },
      child: Container(
        width: 40,
        height: 40,
        decoration: BoxDecoration(
          shape: BoxShape.circle,
          border: Border.all(
            color: Colors.white.withOpacity(0.4),
            width: 2,
            style: BorderStyle.solid,
          ),
        ),
        child: Center(
          child: R.img(
            BaseAssets.close_icon_svg,
            width: 24,
            height: 24,
            color: Colors.white,
            package: ComponentManager.MANAGER_BASE_CORE,
          ),
        ),
      ),
    );
  }

  void _onItemLayout(double radio) {
    if (_realRadio != radio) {
      _realRadio = radio;
      setState(() {});
    }
  }

  String _getPrice(int price) {
    return (price / 100).toStringAsFixed(0);
  }
}
