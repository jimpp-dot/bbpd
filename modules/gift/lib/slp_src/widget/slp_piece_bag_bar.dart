...
class SlpPieceBagBar extends StatefulWidget {
  final int id;
  final BbGiftPanelGlobalPiece? globalPiece;
  final int rid;
  final VoidCallback? onCombine;

  const SlpPieceBagBar({
    Key? key,
    this.id = 0,
    this.globalPiece,
    this.rid = 0,
...
class PieceBabBarState extends State<SlpPieceBagBar> {
  int _selectNum = 1;
  ...
  Widget _buildTipsBar() {
    String giftImgUrl = widget.globalPiece?.maxGiftImage ?? '';
    return Row(
      children: <Widget>[
        Text(
          K.gift_now_compound_maybe_gain,
          textScaleFactor: 1.0,
          style: TextStyle(
            fontSize: 15,
            color:
        ...
        InkWell(
          onTap: () {
            ...
            BaseWebviewScreen.show(context,
                url: url, title: K.gift_compound_rule);
          },
          child: Text(
            K.gift_compound_rule_,
            style: TextStyle(
                color: widget.inRoom
  ...
  Widget _buildRecentGiftBar() {
    List<BbGiftPanelGpRecentGifts>? recentGifts =
        widget.globalPiece?.recentGifts;
    if (recentGifts == null || recentGifts.isEmpty) return Container();

    BbGiftPanelGpRecentGifts recentGift = recentGifts[0];
    if (!recentGift.hasCimage() ||
        !recentGift.hasIcon() ||
        !recentGift.hasName() ||
        !recentGift.hasTimeText()) {
      return Container();
    }

    String cImage = recentGift.cimage;
    String userIcon = recentGift.icon;
    String userName = recentGift.name;
    String diffTime = recentGift.timeText;
    ...
    return Container(
      ...
      child: Row(
        children: <Widget>[
          ...
          Text(K.gift_compound_char_gain,
              maxLines: 1,
              style: R.textStyle.caption.copyWith(color: textColor)),
          CachedNetworkImage(
