...
class FlutterWebview {
  ...
  final _onDestroy = StreamController<void>.broadcast();
  ...
  final _onError = StreamController<WebViewError>.broadcast();

  static FlutterWebview? _instance;

  factory FlutterWebview() => _instance ??= FlutterWebview._();
  ...
  Future<void> _handleMessages(MethodCall call) async {
    switch (call.method) {
      ...
      case 'onError':
        _onError.add(
            WebViewError(call.arguments['code'], call.arguments['message']));
        break;
    }
  }

  /// Listening the OnDestroy LifeCycle Event for Android
  Stream<void> get onDestroy => _onDestroy.stream;
...
class WebViewError {
  ...
  WebViewError(this.code, this.msg);
}
